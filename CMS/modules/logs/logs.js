...
$(function () {
  ...
  function getActionLabel(log) {
    const raw =
      log && typeof log.action !== "undefined" ? String(log.action) : "";
  ...
  function normalizeLogs(logs) {
    ...
    return logs
      .map(function (item) {
        ...
        const slug =
          item && item.action_slug
            ? String(item.action_slug)
            : slugifyAction(label);
        const context = item && item.context ? String(item.context) : "page";
        const rawDetails =
          item && typeof item.details !== "undefined" ? item.details : [];
        let details = [];
  ...
  function renderFilters(logs) {
    ...
    if (
      ...
      !actions.some(function (action) {
        return action.slug === currentAction;
      })
    ) {
      currentAction = "all";
    }
    ...
    const allBtn = $('<button type="button" class="logs-filter-btn"></button>')
      ...
      .append(
        $('<span class="logs-filter-count" id="logsAllCount"></span>').text(
          logs.length,
        ),
      );
    ...
    actions.slice(0, 4).forEach(function (action) {
      const btn = $('<button type="button" class="logs-filter-btn"></button>')
        ...
        .append(
          $('<span class="logs-filter-count"></span>')
            .attr("data-filter-count", action.slug)
            .text(action.count),
        );
  ...
  function buildRow(log) {
    ...
    const pageTitle =
      log.page_title || (context === "system" ? "System activity" : "Unknown");
    ...
    return (
      ...
      '<time datetime="' +
      exact +
      '" class="logs-activity-time" title="' +
      escapeHtml(absolute) +
      '">' +
      escapeHtml(relative) +
      "</time>" +
  ...
  function updateTimeline(logs) {
    if (!logs.length) {
      ...
      const message =
        hasAnyLogs && isFiltered
          ? "No activity matches the current filters."
          : "No activity recorded yet.";
      const hint =
        hasAnyLogs && isFiltered
          ? "Try selecting a different editor or action."
          : "Updates will appear here as your team edits content.";
      timeline.html(
        '<div class="logs-empty"><i class="fas fa-clipboard-list" aria-hidden="true"></i><p>' +
          escapeHtml(message) +
          '</p><p class="logs-empty-hint">' +
          escapeHtml(hint) +
          "</p></div>",
      );
      return;
  ...
  function updateStats(logs) {
    ...
    const heroTitle = heroTimestamp
      ? formatAbsolute(heroTimestamp)
      : "No recent activity";
    ...
    if (statsEls.topActionLabel.length && statsEls.topActionCount.length) {
      if (actions.length) {
        ...
        statsEls.topActionCount.text(
          actions[0].count + (actions[0].count === 1 ? " entry" : " entries"),
        );
      } else {
  ...
  function renderUserFilter(logs) {
    ...
    logs.forEach(function (log) {
      const key =
        log.user_key || userKeyFromNormalized(normalizeUserValue(log.user));
      const label =
        log.user_display ||
        userDisplayFromNormalized(normalizeUserValue(log.user));
      if (!userMap.has(key)) {
  ...
  if (refreshBtn.length) {
    refreshBtn.on("click", function () {
      ...
      $.getJSON(endpoint)
        ...
        .fail(function () {
          const alert = $(
            '<div class="logs-inline-alert" role="status">Unable to refresh activity right now.</div>',
          );
